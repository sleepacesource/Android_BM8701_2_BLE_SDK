...
public class SearchBleDeviceActivity extends BaseActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == BleConfig.REQCODE_OPEN_BT) {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            mHelper.setMtu(device.getAddress(), 509, new IResultCallback<Integer>() {
                ...
                public void onResultCallback(IDeviceManager manager, CallbackData<Integer> cd) {
                    ...
                    if (cd.getCallbackType() == BM8701DeviceManager.CallbackType.MTU) {
                        if (cd.isSuccess()) {
                            //set mtu success
            ...
            //  String deviceName, String address, int timeout
            mHelper.connectDevice(device.getDeviceName(), device.getAddress(), 20000, connectCallback);
        }
    };

    private IResultCallback connectCallback = new IResultCallback() {
        @Override
        public void onResultCallback(IDeviceManager manager, final CallbackData cd) {
            ...
            runOnUiThread(new Runnable() {
                public void run() {
                    ...
                    if (cd.isSuccess()) {
                        Toast.makeText(SearchBleDeviceActivity.this, device.getDeviceName() + "已连接", Toast.LENGTH_SHORT).show();
                        MainActivity.deviceInfos.get(deviceIndex).setDeviceName(device.getDeviceName());
                        MainActivity.deviceInfos.get(deviceIndex).setAddress(device.getAddress());
                        finish();
                    } else {
        ...
        public void onLeScan(final BluetoothDevice device, int rssi, final byte[] scanRecord) {// Z1-140900000
        	runOnUiThread(new Runnable() {
				...
				public void run() {
		            ...
		            if(!TextUtils.isEmpty(deviceName) && deviceName.startsWith("BM")){
